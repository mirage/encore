...
let flush k0 encoder =
  if encoder.stk > 0
  then ( if Bigstringaf.length encoder.buffer - encoder.pos > 0
         then
           k0 { encoder with buffer= Bigstringaf.sub
                                    ~off:encoder.pos
  ...
  then
    let rec k1 n =
      if n < encoder.pos
      then Partial { buffer= encoder.buffer
                   ; off= n
                   ; len= encoder.pos - n
...
let rec write_char chr k encoder =
  ...
  then ( Bigstringaf.set encoder.buffer encoder.pos chr
...
let rec write_string str k encoder =
  ...
  then ( Bigstringaf.blit_from_string
           str ~src_off:0
           encoder.buffer ~dst_off:encoder.pos
           ~len
...
type -'a t =
  { run : (encoder -> state) -> encoder -> 'a -> state } [@@unbox]

let finish encoder = flush (fun _ -> Done) encoder
let error encoder = flush (fun e -> if e.stk > 0 then raise Fail else Fail) encoder
...
let emit_string value d =
  ...
  let rec go = function
    | Partial { buffer; off; len; continue; } ->
      ...
      Buffer.add_string buf str ; go (continue ~committed:len)
...
let string str =
  { run= fun k e v -> assert (v = str) ; write_string v k e }
...
let put p n =
  { run= fun k e v ->
        ...
        then write_string v k e else error e }

let at_least_put p n =
  { run= fun k e v ->
        if string_for_all_while n p v
        then write_string v k e else error e }
...
let range ~a ~b p =
  { run= fun k e v ->
        ...
        let y = ref a in
        while !y < max && p v.[!y] do incr y done ;
...
let map x f =
  { run= fun k e v ->
        try x.run k e (f v)
        with Bij.Bijection -> error e }

let commit =
  { run= fun k e () -> flush k e }

let peek a b =
  { run= fun k e -> function Either.L x -> a.run k e x | Either.R y -> b.run k e y }

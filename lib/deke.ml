...
let _take_back_node n = match n.cell with
  | One x -> (true, x)
...
let _take_front_node n = match n.cell with
  | One x -> (true, x)
  | Two (x, y) -> n.cell <- One y ; (false, x)
  | Three (x, y, z) -> n.cell <- Two (y, z) ; (false, x)

let pop d = match d.cur with
  | Empty -> raise Empty
  | Node cur ->
    if cur.prev == cur
    then

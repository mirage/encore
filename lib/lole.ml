...
module RBQ (V : VALUE) = struct
  ...
  let pp ppf t =
    Fmt.pf ppf "{ @[<hov>a = %a;@ c = %d;@ m = %d;@ q = %a;@] }"
      Fmt.(Dump.array V.pp)
      t.a t.c t.m (Queue.dump Fmt.int) t.q
...
let pp_bigstring =
  pp_scalar ~get:Bigarray_compat.Array1.get ~length:Bigarray_compat.Array1.dim
  ...
  let pequal a b =
    ...
    | { buffer = Buffer.Bigstring a; _ }, { buffer = Buffer.Bigstring b; _ }
      -> (
        ...
        | Some (len, 0, 0) ->
            Bigarray_compat.Array1.dim a = len
            && Bigarray_compat.Array1.dim b = len
        | _ -> false)
    | _, _ -> false
...
let shift n t =
  ...
  ( lst,
    shift_flushes (IOVec.lengthv lst) t |> fun t ->
  ...
  let rec aux t = function
    ...
    | (length, off, len, buffer) :: r ->
        schedule
          ...
          ~length ?off ?len ~buffer:identity buffer t in
  aux t l
...
let schedulev_bigstring k l t =
  let rec aux t = function
    | [] -> continue k t
    | buffer :: r ->
        schedule_bigstring (fun t -> (aux [@tailcall]) t r) t buffer in
  aux t l
...
let rec write k ~blit ~length ?(off = 0) ?len buffer t =
  ...
  then
    ...
    let areas, write =
      RBA.N.push_exn t.write ~blit ~length ~off ~len:available buffer in
...
let write_char =
  ...
  let blit src src_off dst dst_off len =
    assert (src_off = 0) ;
    assert (len = 1) ;
    Bigstringaf.set dst dst_off src in
...
let write_uint8 =
  ...
  let blit src src_off dst dst_off len =
    assert (src_off = 0) ;
    assert (len = 1) ;
    Bigstringaf.set dst dst_off (Char.chr (src land 0xff)) in
  fun a k t -> write k ~length ~blit ~off:0 ~len:1 a t
...
module BE = struct
  ...
  let write_uint16 =
    ...
    let blit src src_off dst dst_off len =
      assert (src_off = 0) ;
      assert (len = 2) ;
      Bigstringaf.set_int16_be dst dst_off src in
  ...
  let write_uint32 =
    ...
    let blit src src_off dst dst_off len =
      assert (src_off = 0) ;
      assert (len = 4) ;
      Bigstringaf.set_int32_be dst dst_off src in
  ...
  let write_uint64 =
    ...
    let blit src src_off dst dst_off len =
      assert (src_off = 0) ;
      assert (len = 8) ;
      Bigstringaf.set_int64_be dst dst_off src in
...
module LE = struct
  ...
  let write_uint16 =
    ...
    let blit src src_off dst dst_off len =
      assert (src_off = 0) ;
      assert (len = 2) ;
      Bigstringaf.set_int16_le dst dst_off src in
    fun a k t -> write k ~length ~blit ~off:0 ~len:2 a t
  ...
  let write_uint32 =
    ...
    let blit src src_off dst dst_off len =
      assert (src_off = 0) ;
      assert (len = 4) ;
      Bigstringaf.set_int32_le dst dst_off src in
    fun a k t -> write k ~length ~blit ~off:0 ~len:4 a t
  ...
  let write_uint64 =
    ...
    let blit src src_off dst dst_off len =
      assert (src_off = 0) ;
      assert (len = 8) ;
      Bigstringaf.set_int64_le dst dst_off src in
    fun a k t -> write k ~length ~blit ~off:0 ~len:8 a t
end
